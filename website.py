...
class Website():
    ...
    #     stats_thread = threading.Timer(10, self.get_stats, args=[timeframe,database_name, stats_display_time])
    #     stats_thread.start()
    #     #os.system('clear')
    #     # Compute the statistiques of some metrics over a timeframe
    #     # print("Stats of the website : ", self.URL, "over ", timeframe/60, "minutes ")
    #     printStr = '\n\033[4;93m ####### Statistiques of the website {} ########\033[0m'.format(self.URL)
    ...
    # def get_stats(self, timeframe, database_name,stats_display_time , printStr):
    #     stats_thread = threading.Timer(10, self.get_stats, args=[timeframe,database_name, stats_display_time])
    #     stats_thread.start()
    #     #os.system('clear')
    #     # Compute the statistiques of some metrics over a timeframe
    ...
    def get_stats(self, timeframe, database_name):
        ...
        # Save on the table 'alerts_table' on our database if there is a detected alert and when it's recovered
        if timeframe == 120: # 2 minutes
            if availability < 0.8 and self.isDown == False:
